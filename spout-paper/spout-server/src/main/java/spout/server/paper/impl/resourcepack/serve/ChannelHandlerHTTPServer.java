package spout.server.paper.impl.resourcepack.serve;
...
public final class ChannelHandlerHTTPServer {
    ...
    static {
        MIN_PACKET_LENGTH = 5 + Math.min(
            ResourcePackServing.VANILLA_PACK_PATH.length(),
            ResourcePackServing.CLIENT_MOD_PACK_PATH.length()
        );
        int index = 0;
        while (ResourcePackServing.VANILLA_PACK_PATH.charAt(index) == ResourcePackServing.CLIENT_MOD_PACK_PATH.charAt(index)) {
            index++;
        }
    ...
    private static boolean tryHandle(ChannelHandlerContext ctx, ByteBuf byteBuf) {
        ...
        try {
            ...
            byte[] dataToSend;
            if (path.equals(ResourcePackServing.VANILLA_PACK_PATH)) {
                dataToSend = ResourcePackServing.vanillaPackBytes;
            } else if (path.equals(ResourcePackServing.CLIENT_MOD_PACK_PATH)) {
                dataToSend = ResourcePackServing.clientModPackBytes;
            } else {
                // Not a known path
                ctx.close();
                return true;
